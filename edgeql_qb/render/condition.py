...
def render_filters(
    ...
    conditions = [
        render_condition(
            filter_.to_infix_notation(index),
            index,
            generator,
        )
...
def _(
    ...
    arg_renderers = [
        render_condition(arg, index, generator)
...
@render_condition.register
def _(expression: Node, index: int, generator: Iterator[int]) -> RenderedQuery:
    if expression.right is None:
        return combine_many_renderers(
            RenderedQuery(expression.op),
            render_condition(expression.left, index, generator),
        )
    return render_binary_node(
        left=render_condition(expression.left, index, generator),
        right=render_condition(expression.right, index, generator),
